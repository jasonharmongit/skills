...
metadata:
  dependents: solutionize, modify-sketch, implement-next-phase
---
...
- Set each todo's `status` to `pending` when the sketch is first written.
...
- **No tests:** do not include test files, test cases, test-only work, or test phases anywhere in the sketch.
...
Do not write vague bullets like "update logic" or "wire this up". Every bullet must name the exact symbol or code area being changed. For an existing function or module attribute, use its actual name (for example **twiml_voicemail/2**). For a new function, worker, migration, or file, use the concrete name you are proposing (for example **Surge.PhoneNumbers.CopyVoicemailGreeting** or **presigned_voicemail_greeting_url/1**) — never generic placeholders like **new_function_name** or **updated_function_name**.
